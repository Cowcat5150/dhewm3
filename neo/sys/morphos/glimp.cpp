...
static void SetSDLIcon()
{
	...
	SDL_Surface* icon = SDL_CreateRGBSurfaceFrom((void*)d3_icon.pixel_data, d3_icon.width, d3_icon.height,
			d3_icon.bytes_per_pixel*8, d3_icon.bytes_per_pixel*d3_icon.width, rmask, gmask, bmask, amask);
...
bool GLimp_Init(glimpParms_t parms)
{
	...
	for (int i = 0; i < 16; i++)
	{
		...
		window = SDL_CreateWindow(ENGINE_VERSION, SDL_WINDOWPOS_UNDEFINED_DISPLAY(displayIndex),
				SDL_WINDOWPOS_UNDEFINED_DISPLAY(displayIndex), parms.width, parms.height, flags);
	...
	SDL_GL_GetAttribute(SDL_GL_RED_SIZE, &j);
	common->Printf("SDL_GL_RED_SIZE %i\n", j);

	SDL_GL_GetAttribute(SDL_GL_GREEN_SIZE, &j);
	common->Printf("SDL_GL_GREEN_SIZE %i\n", j);

	SDL_GL_GetAttribute(SDL_GL_BLUE_SIZE, &j);
	common->Printf("SDL_GL_BLUE_SIZE %i\n", j);

	SDL_GL_GetAttribute(SDL_GL_DEPTH_SIZE, &j);
	common->Printf("SDL_GL_DEPTH_SIZE %i\n", j);

	SDL_GL_GetAttribute(SDL_GL_STENCIL_SIZE, &j);
	common->Printf("SDL_GL_STENCIL_SIZE %i\n", j);

	SDL_GL_GetAttribute(SDL_GL_ALPHA_SIZE, &j);
	common->Printf("SDL_GL_ALPHA_SIZE %i\n", j);

	SDL_GL_GetAttribute(SDL_GL_DOUBLEBUFFER, &j);
	common->Printf("SDL_GL_DOUBLEBUFFER %i\n", j);
	*/
...
GLExtension_t GLimp_ExtensionPointer(const char *name) {
	...
	//printf("GLimp_ExtensionPointer (%s)\n", name);
