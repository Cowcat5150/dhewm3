...
int Sys_ListFiles( const char *directory, const char *extension, idStrList &list )
{
	...
	//common->Printf("Sys_ListFiles: in\n");

	list.Clear();

	//common->Printf("Sys_ListFiles: 1 clear\n");
	...
	//common->Printf("Sys_ListFiles: opendir %s\n", directory);
	...
	//common->Printf("Sys_ListFiles: 2 opendir OK\n");
	...
	while ((d = readdir(fdir)) != NULL)
	{
        ...
        //common->Printf("Sys_ListFiles: readir %s\n", d->d_name);

		list.Append(d->d_name);
	}

	//common->Printf("Sys_ListFiles: 3 readdir\n");

	closedir(fdir);

	//common->Printf("Sys_ListFiles: 4 closedir\n");

	if ( debug ) {
		common->Printf( "Sys_ListFiles: %d entries in %s\n", list.Num(), directory );
	}

	//common->Printf("Sys_ListFiles: out\n");
...
uintptr_t Sys_DLL_Load( const char *path )
{
	...
	void *handle = dllLoadLibrary( (char *)path, 0 );
